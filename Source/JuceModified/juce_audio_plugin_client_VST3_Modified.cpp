...
public:
    ...
    enum InternalParameters
    {
        ...
        paramMidiProgramChangeBase   = 0x6d706300  // 'mpc\0' .. 'mpc\x0F'
    };

    static constexpr int kNumMidiChannels = 16;

    static bool isMultiChannelProgramChangeParamID (Vst::ParamID id) noexcept
    {
        return id >= static_cast<Vst::ParamID> (paramMidiProgramChangeBase)
            && id <  static_cast<Vst::ParamID> (paramMidiProgramChangeBase) + kNumMidiChannels;
    }

    static int getMidiChannelForProgramChangeParamID (Vst::ParamID id) noexcept
    {
        return static_cast<int> (id - static_cast<Vst::ParamID> (paramMidiProgramChangeBase));
    }

    static Vst::ParamID getProgramChangeParamIDForChannel (int channel) noexcept
    {
        return static_cast<Vst::ParamID> (paramMidiProgramChangeBase) + static_cast<Vst::ParamID> (channel);
    }

    //==============================================================================
    Steinberg::int32 getNumChannelUnits() const noexcept
    {
        return kNumMidiChannels;
    }

    Steinberg::int32 getFirstChannelUnitIndex() const noexcept
    {
        return 1 + parameterGroups.size();
    }

    static Vst::UnitID getChannelUnitID (int channel) noexcept
    {
        return static_cast<Vst::UnitID> (0x7FFFFF00 + channel);
    }

    static Vst::ProgramListID getChannelProgramListID (int channel) noexcept
    {
        return static_cast<Vst::ProgramListID> (0x7FFFFE00 + channel);
    }

    Steinberg::int32 PLUGIN_API getUnitCount() override
    {
        return parameterGroups.size() + 1 + getNumChannelUnits();
    }
    ...
    tresult PLUGIN_API getUnitInfo (Steinberg::int32 unitIndex, Vst::UnitInfo& info) override
    {
        ...
        const auto firstChannelUnitIndex = getFirstChannelUnitIndex();

        if (unitIndex >= firstChannelUnitIndex
             && unitIndex <  firstChannelUnitIndex + getNumChannelUnits())
        {
            const int channel = unitIndex - firstChannelUnitIndex;

            info.id             = getChannelUnitID (channel);
            info.parentUnitId   = Vst::kRootUnitId;
            info.programListId  = getChannelProgramListID (channel);

            toString128 (info.name, "MIDI Ch " + String (channel + 1));
    ...
    {
        const auto base = (audioProcessor->getNumPrograms() > 0) ? 1 : 0;
        return base + getNumChannelUnits();
    }

    tresult PLUGIN_API getProgramListInfo (Steinberg::int32 listIndex, Vst::ProgramListInfo& info) override
    {
        const auto hasFactoryList = (audioProcessor->getNumPrograms() > 0);

        if (hasFactoryList && listIndex == 0)
        {
        ...
        const auto channelIndex = listIndex - (hasFactoryList ? 1 : 0);

        if (channelIndex >= 0 && channelIndex < getNumChannelUnits())
        {
            info.id = getChannelProgramListID (channelIndex);
            info.programCount = 128; // standard MIDI program range

            toString128 (info.name, "MIDI Ch " + String (channelIndex + 1) + " Programs");
    ...
    tresult PLUGIN_API getProgramName (Vst::ProgramListID listId, Steinberg::int32 programIndex, Vst::String128 name) override
    {
        ...
        for (int ch = 0; ch < getNumChannelUnits(); ++ch)
        {
            if (listId == getChannelProgramListID (ch)
                && isPositiveAndBelow ((int) programIndex, 128))
            {
                toString128 (name, "Program " + String (programIndex));
                return kResultTrue;
            }
        }
    ...
    tresult PLUGIN_API getUnitByBus (Vst::MediaType type, Vst::BusDirection dir, Steinberg::int32, Steinberg::int32 channel, Vst::UnitID& unitId) override
    {
        if (type == Vst::kEvent && dir == Vst::kInput
             && channel >= 0 && channel < kNumMidiChannels)
        {
            unitId = getChannelUnitID (channel);
            return kResultOk;
        }
...
public:
    ...
    //==============================================================================
    struct MidiChannelProgramChangeParameter final : public Vst::Parameter
    {
        MidiChannelProgramChangeParameter (Vst::ParamID vstParamID, int midiChannel, Vst::UnitID unitID)
            : channel (midiChannel)
        {
            info.id = vstParamID;

            const auto label = "MIDI Ch " + String (midiChannel + 1) + " Program";
            toString128 (info.title, label);
            toString128 (info.shortTitle, "Ch" + String (midiChannel + 1) + " Prg");
            toString128 (info.units, "");

            info.stepCount              = 127;
            info.defaultNormalizedValue = 0.0;
            info.unitId                 = unitID;
            info.flags                  = Vst::ParameterInfo::kIsProgramChange;
        }

        ~MidiChannelProgramChangeParameter() override = default;

        void toString (Vst::ParamValue value, Vst::String128 result) const override
        {
            toString128 (result, "Program " + String (roundToInt (value * info.stepCount)));
        }

        bool fromString (const Vst::TChar* text, Vst::ParamValue& outValueNormalized) const override
        {
            const auto str = String (CharPointer_UTF16 (reinterpret_cast<const CharPointer_UTF16::CharType*> (text)));
            const auto trimmed = str.fromFirstOccurrenceOf ("Program", false, true).trim();
            const auto value = trimmed.getIntValue();

            if (value >= 0 && value <= info.stepCount)
            {
                outValueNormalized = static_cast<Vst::ParamValue> (value) / static_cast<Vst::ParamValue> (info.stepCount);
                return true;
            }
    ...
    private:
        ...
        JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiChannelProgramChangeParameter)
    };
    ...
    void installAudioProcessor (const VSTComSmartPtr<JuceAudioProcessor>& newAudioProcessor)
    {
        ...
        if (auto* pluginInstance = getPluginInstance())
        {
            ...
            if (parameters.getParameterCount() <= 0)
            {
                ...
                for (int ch = 0; ch < JuceAudioProcessor::kNumMidiChannels; ++ch)
                {
                    const auto paramID = JuceAudioProcessor::getProgramChangeParamIDForChannel (ch);
                    const auto unitID  = JuceAudioProcessor::getChannelUnitID (ch);
                    parameters.addParameter (new MidiChannelProgramChangeParameter (paramID, ch, unitID));
                }
            }
    ...
    void processParameterChanges (Vst::IParameterChanges& paramChanges)
    {
        ...
        for (Steinberg::int32 i = 0; i < numParamsChanged; ++i)
        {
            if (auto* paramQueue = paramChanges.getParameterData (i))
            {
                ...
                if (JuceAudioProcessor::isMultiChannelProgramChangeParamID (vstParamID))
                {
                    const auto channel = JuceAudioProcessor::getMidiChannelForProgramChangeParamID (vstParamID);

                    for (Steinberg::int32 point = 0; point < numPoints; ++point)
                    {
                        if (const auto change = getPointFromQueue (paramQueue, point))
                        {
                            const auto programNumber = jlimit (0, 127, (int) std::round (change->value * 127.0));
                            midiBuffer.addEvent (MidiMessage::programChange (channel + 1, programNumber),
                                                 (int) change->offsetSamples);
                        }
                    }
